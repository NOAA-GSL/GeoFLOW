...
// RETURNS: none.
//**********************************************************************************
void GInitSFactory::static void init(const geoflow::tbox::PropertyTree& ptree, EqnBasePtr &eqn_ptr, GGrid &grid, Time &time, State &ub, State &u)
{
  ...
  if      ( "inits_icosgaussburgers"  == sinit ) {
    ginits::impl_icosgauss       (vtree, eqn_ptr, grid, time, ub, u);
  }
  else if ( "inits_boxplaneburgers"   == sinit ) {
    ginits::impl_boxplaneburgers (vtree, eqn_ptr, grid, time, ub, u);
  }
  else if ( "inits_boxdirgauss"        == sinit ) {
    ginits::impl_boxdirgauss     (vtree, eqn_ptr, grid, time, ub, u);
  }
  else if ( "inits_boxpergauss"        == sinit ) {
    ginits::impl_boxpergauss     (vtree, eqn_ptr, grid, time, ub, u);
  }
  else if ( "inits_nwave"              == sinit ) {
    ginits::impl_nwave           (vtree, eqn_ptr, grid, time, ub, u);
  }
  else                                        {
