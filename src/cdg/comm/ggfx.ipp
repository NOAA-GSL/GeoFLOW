...
//          there is an error in the gather/scatter combination.
//************************************************************************************
template<typename T> GBOOL GGFX::doOp(GTVector<T> &u, GTVector<GSIZET> &iind, GGFX_OP op)
{
  assert( std::is_arithmetic<T>::value && "Illegal template type");
...
#endif
  bret = localGS(u, iind, iOpL2RIndices_, nOpL2RIndices_, op, recvBuff_.data().data());
  if ( !bret ) {
    std::cout << serr << "localGS (2) failed " << std::endl;
    exit(1);
  }
...
//************************************************************************************
template<typename T> 
GBOOL GGFX::localGS(GTVector<T> &qu, GSZMatrix &ilocal, GSZBuffer &nlocal, GGFX_OP op, GDOUBLE *qop)
{
...
//************************************************************************************
template<typename T> 
GBOOL GGFX::localGS(GTVector<T> &qu, GTVector<GSIZET> &iind, 
...
//************************************************************************************
template<typename T> 
GBOOL GGFX::dataExchange(GTVector<T> &u)
{
  assert( std::is_arithmetic<T>::value && "Illegal template type");
...
//************************************************************************************
template<typename T> 
GBOOL GGFX::dataExchange(GTVector<T> &u, GTVector<GSIZET> &iind)
{
  assert( std::is_arithmetic<T>::value && "Illegal template type");
...
GBOOL GGFX::dataExchange(T *&u, GSIZET nu)
{
 ...
 // if ( iOpL2RTasks_[j] == rank_ ) continue;
  ...
  return bret;
} // end of dataExchange (3)


//************************************************************************************
//************************************************************************************
// METHOD : dataExchange (4)
// DESC   : Perform data exchange between procs by using asynchronous
//              recvs...
// ARGS   :
//          u    : field data 
